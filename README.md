...
A long-only quantitative trading bot # SG-HK Quant Bot
...
├── bot/
│   ├── config/
│   │   └── settings.py          # Strategy and execution parameters
│   ├── execution/
│   │   └── broker.py            # Roostoo order and balance wrapper
│   ├── logs/
│   │   ├── bot.log              # Runtime activity log
│   │   └── trades.csv           # Structured trade log
│   ├── strategies/
│   │   └── vwap_reversion.py    # Indicator and signal logic
│   ├── utils/
│   │   ├── indicators.py        # Shared indicator helpers
│   │   ├── risk.py              # Position sizing and risk helpers
│   │   └── trade_logger.py      # CSV trade logging helper
│   └── state/
│       └── runtime_state.json   # Position state persisted across restarts
├── main.py                      # Live mock trading entry point
...
> Actual file names may differ slightly depending on the final submitted structure, but the design principle is the same: **strategy logic, execution logic, config, and utilities are cleanly separated**.
...
built for the **SG vs HK University Web3 Quant Hackathon**.
